...
public class AdminChatServiceImpl implements AdminChatService {
    ...
    public long getTotalChatCount(Instant start, Instant end) {
        return repository.countByTypeAndTimestampBetween(ActivityType.CHAT, start, end);
    }
    public Page<UserActivityLog> getRecentChatLogs(Instant start, Instant end, Pageable pageable) {
        return repository.findByTypeAndTimestampBetween(ActivityType.CHAT, start, end, pageable);
    }

    public List<DailyTrendResponse> getChatDailyTrend(Instant start, Instant end) {
